...
def calc_score(fileprefix,values,gridscale,xn,yn,zn,origin,mol,vdw_dict,fileh,return_vals,Hchoice,radius_pad):
# file - log file
# values gist values 
# gridscale - the grid spacing
    ...
    for atom_i,atom in enumerate(mol.atom_list):
        ...
        if (radius_pad>0.0):
            # this lets us pad the radius by some value.  if 8.0 then at Angstroms are added to the atom radius
            radius = radius + radius_pad
